//06_윈도우핸들얻기.cpp
/*
[어떤사건이 발생했을 때] -> [어떤 일을 하고싶다]
키보드 F 가 눌러졌을 때 -> 계산기를 화면에서 지우고 싶다.
*/
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		if (wParam == 'F')
		{
			SetWindowText(hwnd, TEXT("F 클릭"));
			HWND h = FindWindow(0, TEXT("계산기"));
			if (h == 0)
			{
				SetWindowText(hwnd, TEXT("못찾음"));
			}
			else
			{
				SetWindowText(hwnd, TEXT("찾음"));
