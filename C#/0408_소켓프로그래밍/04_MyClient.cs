...
namespace MYClient
{
    ...
    class MyClient2 : IDisposable
    {
        ...
        LogMessage LogCallback { get; set; } = null;
        PacketMessage PacketCallback { get; set; } = null;
        ...
        public bool Start(string ip, int port, LogMessage log, PacketMessage pack)
        {
            try
            {
                ...
                workThread.IsBackground = true; //스레드를 백그라운드 스레드로 설정
                workThread.Start();
                LogCallback(socket, string.Format("[소켓 생성 성공]: {0}주소, {1}포트 접속", ip, port));
                return true;

            }
            catch (Exception e)
            {
                LogCallback(socket, string.Format("[소켓 생성 오류]: " + e.Message));
                return false;
            }
        }
        //recv 전용 스레드
        private void WorkThread()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    PacketCallback(socket, string.Format($"[수신]: {msg}"));
                }
                catch (Exception e)
                {
                    LogCallback(socket, string.Format("[통신 오류]: " + e.Message));
                    break;
                }
            }
        }
