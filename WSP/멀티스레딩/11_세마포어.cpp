...
DWORD WINAPI ThreadFunc1(LPVOID param)
{
	...
	for (int i = 0; i < 100; i++)
	{
		DWORD r = WaitForSingleObject(hSemaphore, INFINITE);//
		g_x = 100;
		g_x++;
		TCHAR buf[20];
		wsprintf(buf, _T("%d"), g_x);
		TextOut(hdc, 100, 200, buf, _tcslen(buf));
		LONG old;
		ReleaseSemaphore(hSemaphore, 1, &old);//
		Sleep(20);
		//InvalidateRect((HWND)param, 0, TRUE);
	}
...
DWORD WINAPI ThreadFunc2(LPVOID param)
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		TCHAR buf[20];
		wsprintf(buf, _T("%d"), g_x);
		TextOut(hdc, 100, 100, buf, _tcslen(buf));
		LONG old;
		ReleaseSemaphore(hSemaphore, 1, &old);//
		Sleep(20);
		//InvalidateRect((HWND)param, 0, TRUE);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		hSemaphore = CreateSemaphore(0, 3, 3, TEXT("shema"));
